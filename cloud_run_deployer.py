...
class CloudRunDeployer:
    ...
    def __init__(
        ...
        self.logger.info(
            f"CloudRunDeployer initialized with project_id='{self.project_id}', region='{self.region}'"
        )
    ...
    def _deploy_with_client_libraries(self, service_name: str, source_dir: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"Service {service_name} does not exist, will create new"
                )
    ...
    def _deploy_with_gcloud_cli(self, service_name: str, source_dir: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                set_project_cmd, check=True, capture_output=True, text=True
            )
            self.logger.debug(
                f"Set project result - stdout: {result.stdout}, stderr: {result.stderr}, returncode: {result.returncode}"
            )
            ...
            print(
                f"   • Visit: https://console.cloud.google.com/cloud-build/builds?project={self.project_id}"
            )
            ...
            print(f"\n❌ {error_msg}")
            print(
                "💡 The deployment is taking longer than expected. This might be due to:"
            )
            print("   • Large Docker image build times")
            print("   • Network connectivity issues")
            print("   • Google Cloud Build queue delays")
            print(
                f"💡 You can check the status manually with: gcloud run services list --project={self.project_id}"
            )
            self.logger.error(error_msg)
            return False
    ...
    def _fetch_build_logs(self, service_name: str):
        ...
        if self.build_client:
            try:
                ...
                self.logger.warning(
                    f"Failed to fetch build logs with client libraries: {e}"
                )
        ...
        try:
            ...
            if build_list:
                ...
                for build in build_list:
                    build_id = build.id
                    print(
                        f"   Fetching logs for build: {build_id} (Status: {build.status.name})"
                    )
    ...
    def _fetch_build_logs_with_gcloud(self, service_name: str):
        ...
        try:
            ...
            self.logger.debug(
                f"Build list result - returncode: {result.returncode}, stdout: {result.stdout}, stderr: {result.stderr}"
            )
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                for build_id in build_ids:
                    if build_id.strip():
                        ...
                        self.logger.debug(
                            f"Running build log command: {' '.join(log_cmd)}"
                        )

                        log_result = subprocess.run(
                            log_cmd, capture_output=True, text=True, timeout=60
                        )
                        self.logger.debug(
                            f"Build log result - returncode: {log_result.returncode}"
                        )
                        ...
                        if log_result.returncode == 0 and log_result.stdout:
                            ...
                            self.logger.warning(
                                f"Could not retrieve logs for build {build_id}"
                            )
            else:
                print("   No recent builds found")
                self.logger.warning("No recent builds found")
